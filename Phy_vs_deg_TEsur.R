...
data<-read.csv("Degree Distribution by Phylostratum TE surf.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  annotation_logticks()+
  labs(title="Degree distribution of genes associated to phylostratum within the TE Surfaceome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Bridgeness Distribution by Phylostratum TE surf.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Bridgeness distribution of genes associated to phylostratum within the TE surfaceome",
...
wilcox.test(x~phylo_bin,data=dat[which(!is.na(match(as.character(dat$phylo_bin),c("2:3","8:19")))),])
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Community Centrality Distribution by Phylostratum TE surf.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Community Centrality distribution of genes associated to phylostratum in the TE surfaceome",
...
wilcox.test(x~phylo_bin,data=dat[which(!is.na(match(as.character(dat$phylo_bin),c("1","4:7")))),])


library(readxl)                                                                              
setwd("B:/Sabrina_2019/")
data<-read_excel("Bridgeness vs Centrality TE surf.xlsx")
...
rownames(newdata)<-as.character(1:219)
newdata2<-newdata
newdata2$Bridgeness<-log10(newdata2$Bridgeness)
newdata2$Centrality<-log10(newdata2$Centrality)
newdata2<-newdata2[which(newdata2$Bridgeness!="-Inf"),]
test<-lm(Bridgeness~Centrality,data=newdata2)
inter<-coef(test)[1]
grad<-coef(test)[2]
newdata2$expected<-NA
for(x in 1:nrow(newdata2)){
  newdata2$expected[x]<-grad*(newdata2$Centrality[x])+inter
}
newdata2$date<-NA
newdata2$date[which(newdata2$Bridgeness>newdata2$expected)]<-1
newdata2$date[which(is.na(newdata2$date))]<-0
newdata$date<-newdata2$date[match(rownames(newdata),rownames(newdata2))]

ggplot(newdata, aes(x=Centrality, y=Bridgeness))+
  geom_point(stat = "identity", aes(col=newdata$date))+
  geom_smooth(method = lm)+
  scale_y_continuous(trans='log10')+
  scale_x_continuous(trans = 'log10')+
  ggtitle("TE surfaceome")
...
write.table(data, file = "TE date party.txt",sep = "\t",row.names = FALSE)
getwd()
