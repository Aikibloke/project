...
data<-read.csv("Degree Distribution by Phylostratum ICM control.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  annotation_logticks()+
  labs(title="Degree distribution of genes associated to phylostratum within the ICM control",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Bridgeness Distribution by Phylostratum ICM control.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Bridgeness distribution of genes associated to phylostratum within the ICM control",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Community Centrality Distribution by Phylostratum ICM control.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Community Centrality distribution of genes associated to phylostratum in the ICM control",
...
library(readxl)                                                                              
setwd("B:/Sabrina_2019/")
data<-read_excel("Bridgeness vs Centrality ICM control.xlsx")
...
ggplot(data, aes(x=Centrality, y=Bridgeness))+
  ...
  scale_x_continuous(trans = 'log10')+
  ggtitle("ICM control")
write.table(data, file = "ICM control date party.txt",sep = "\t",row.names = FALSE)
