...
setwd("B:/Terry/Students/Genetic Age/")
data<-read.csv("Degree Distribution by Phylostratum.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  annotation_logticks()+
  labs(title="Degree distribution of genes associated to phylostratum within the GlcNAcome",
...
data<-read.csv("Betweeness Distribution by Phylostratum GlcNAcome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  geom_point(stat="identity",aes(col=phylo_bin),size=2)+
  geom_smooth(stat="identity",aes(col=phylo_bin))+
  scale_y_continuous(trans = "log10")+
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Betweeness distribution of genes associated to phylostratum within the GlcNAcome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Bridgeness Distribution by Phylostratum GlcNAcome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Bridgeness distribution of genes associated to phylostratum within the GlcNAcome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Community Centrality Distribution by Phylostratum GlcNAcome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Community Centrality distribution of genes associated to phylostratum in the GlcNAcome",
...
library(readxl)
Bridgeness_vs_Centrality_GlcNAcome <- read_excel("Bridgeness vs Centrality GlcNAcome.xlsx")

ggplot(Bridgeness_vs_Centrality_GlcNAcome, aes(x=Centrality, y=Bridgeness))+
  geom_point()+
...
setwd("B:/Sabrina_2019/")
data<-read_excel("Bridgeness vs Centrality GlcNAcome.xlsx")
...
rownames(data)<-as.character(1:2223)
data2<-data
...
write.table(data, file = "GlcNAcome date party.txt",sep = "\t",row.names = FALSE)
