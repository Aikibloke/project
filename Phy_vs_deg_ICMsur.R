...
data<-read.csv("Degree Distribution by Phylostratum ICM surfaceome.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  annotation_logticks()+
  labs(title="Degree distribution of genes associated to phylostratum within the ICM surfaceome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Bridgeness Distribution by Phylostratum ICM surfaceome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Bridgeness distribution of genes associated to phylostratum within the ICM surfaceome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Community Centrality Distribution by Phylostratum ICM surfaceome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Community Centrality distribution of genes associated to phylostratum in the ICM surfaceome",
...
library(readxl)                                                                              
setwd("B:/Sabrina_2019/")
data<-read_excel("Bridgeness vs Centrality ICM surfaceome.xlsx")
...
ggplot(data, aes(x=Centrality, y=Bridgeness))+
  ...
  scale_x_continuous(trans = 'log10')+
  ggtitle("ICM surfaceome")
write.table(data, file = "ICM surf date party.txt",sep = "\t",row.names = FALSE)
...
for(i in 1:length(levels(data$phylo_bin))){
  dls[[i]]<-data[which(data$phylo_bin==levels(data$phylo_bin)[i]),]}
newdata <- data[ !(data$phylo_bin %in% c("1","8:19","4:7")), ]
