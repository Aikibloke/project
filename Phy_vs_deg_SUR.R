data<-data[order(data$Phylostratum),]

setwd("B:/Affiliated people/Sabrina_2019/Degree distributions/")
data<-read.csv("Degree Distribution by Phylostratum Surfaceome.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_y_continuous(trans = "log10", breaks = c(1e-06, 1e-04, 1e-02, 1e+00))+
  scale_x_continuous(trans = "log10", breaks = c(1, 10, 100, 1000))+
  expand_limits(x = c(0,2500), y = c(0.00000000002,1))+
  annotation_logticks()+
...
data<-read.csv("Bridgeness Distribution by Phylostratum Surfaceome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  annotation_logticks()+
  labs(title="Bridgeness distribution of genes associated to phylostratum within the Surfaceome",
...
setwd("B:/Sabrina_2019/")
data<-read.csv("Community Centrality Distribution by Phylostratum Surfaceome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10")+
  annotation_logticks()+
  labs(title="Community Centrality distribution of genes associated to phylostratum in the Surfaceome",
...
setwd("B:/Sabrina_2019/")
data<-read_excel("Bridgeness vs Centrality Surfaceome.xlsx")
...
write.table(data, file = "Surfaceome date party.txt",sep = "\t",row.names = FALSE)
...
for(i in 1:length(levels(data$phylo_bin))){
  dls[[i]]<-data[which(data$phylo_bin==levels(data$phylo_bin)[i]),]}
newdata <- data[ !(data$phylo_bin %in% c("4:7","8:19","2:3")), ]
...
setwd("B:/Affiliated people/Sabrina_2019/Degree distributions/")
data<-read.csv("Degree Distribution by Phylostratum 4-7 Surfaceome vs proteome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
setwd("B:/Affiliated people/Sabrina_2019/Degree Distributions/")
data<-read.csv("Degree Distribution by AluY Yan mural.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
...
a<-ggplot(dat,aes(x=x,y=cumsum))+
  ...
  scale_x_continuous(trans = "log10", breaks = c(1, 10, 100, 1000))+
  expand_limits(x = c(0,3000), y = c(0.000001,1))+
  annotation_logticks()+
  labs(title="Degree distribution of AluY neighbour genes within the polar TE 20k",
       x="Degree",y="Cumulative Frequency",fill="Cell line")+
  theme(plot.title = element_text(size=22),axis.title = element_text(size=18),axis.text = element_text(size=18),legend.text = element_text(size=12),legend.title = element_text(size=12))
# scale_color_manual(values=c("#56B4E9", "#E69F00", "#999999","#458B00"))+
# geom_vline(xintercept = mean(dls[[1]]$degree),col="#56B4E9",size=1.5)+
...
wilcox.test(x~HAR,data=dat[which(!is.na(match(as.character(dat$HAR),c("yes","no")))),])



setwd("B:/Affiliated people/Sabrina_2019/community centrality distribution/")
data<-read.csv("Community centrality distribution by HAR HN Surfaceome vs proteome.csv")
# data<-read.csv("ICM network for final bridgeness dist.csv")
# data<-read.csv("ICM network for final centrality dist.csv")
